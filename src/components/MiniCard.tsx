...
export function MiniCard({ id, showLabel = true }: { id: CardId; showLabel?: boolean }) {
  return <View accessibilityLabel={showLabel ? `${CARD_LABELS[id]}のカード` : '半透明のカラーカード'} style={{ width: 76, height: 94,
    backgroundColor: cardStyle(id), borderRadius: 9, borderWidth: 1, borderColor: 'rgba(38, 56, 48, 0.24)' }}>
    {showLabel && <Text style={{ fontSize: 11, fontWeight: '700', color: '#253E37', marginLeft: 9, marginTop: 2 }}>{CARD_LABELS[id]}</Text>}
