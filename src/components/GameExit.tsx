...
export function GameExit({ children, onExit, enabled, preserveProgress = false }: {
  children: ReactNode;
  onExit: () => void;
  enabled: boolean;
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={() => setVisible(false)}>
      <View style={{ flex: 1, backgroundColor: 'rgba(25,40,30,0.4)', justifyContent: 'center', padding: 24 }}>
        <View accessibilityViewIsModal style={{ backgroundColor: '#F7F4EC', borderRadius: 18, padding: 24, gap: 20, maxWidth: 420, width: '100%', alignSelf: 'center' }}>
          ...
          <Text style={{ color: '#687563', lineHeight: 23 }}>
            {preserveProgress ? '全問モードの進捗は保存され、タイトルから続きに戻れます。未確定のカード配置は保存されません。' : 'ここまでの結果は失われます。制限時間は確認中も進みます。'}
          </Text>
